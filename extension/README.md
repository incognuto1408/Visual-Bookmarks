...
Новая вкладка в стиле референса: **сетка плиток**, поиск с выбором движка, нижняя панель (закрытые вкладки, загрузки, закладки браузера, история), **настройки с вкладками** (оформление / закладки / система), смена **фона** (пресеты Unsplash, свой файл, «фон дня»), демо-панель **Stability**, вход/регистрация через **ваш API** (опционально) или локальный демо-профиль, экспорт/импорт JSON, синхронизация **вашего сервера** (при активной сессии) или **Google Drive**.
...
4. Укажите разрешения и обоснуйте их в форме (для опционального доступа к вашему домену — что данные уходят только на URL, который вводит пользователь).
5. Отправьте на проверку; срок модерации может занять от нескольких часов до нескольких дней.

## Ваш сервер (вход и синхронизация)

В **Настройки → Система** задайте **URL API**. Контракт эндпоинтов (`/auth/login`, `/auth/register`, `/sync/state`) описан в **`SERVER_API.md`**. После сохранения URL Chrome может запросить доступ к этому origin.

Если URL задан, кнопки входа и регистрации на новой вкладке обращаются к серверу; при активной сессии токена синхронизация идёт на сервер (автосохранение с задержкой и при возврате на вкладку). **Google Drive** в этом режиме не используется, чтобы не было двух источников правды.
...
- Опционально `optional_host_permissions` — по запросу пользователя доступ к домену вашего API (`https://*/*`, localhost для разработки).

## Использование

- Плитки: меню «⋯», перетаскивание, счётчик кликов, режимы «иконки» / «скриншоты» (mshots).
- **Настройки**: три вкладки — оформление (тема, фон, сетка, поиск), закладки (лимит, вид, список с добавить/изменить/удалить), система (URL API, Drive, бэкап, сброс).
...
Закладки и настройки хранятся локально в `chrome.storage.local`. При указанном URL API и входе данные синхронизации уходят **на ваш сервер** (см. `SERVER_API.md`). При использовании только Google — к **Drive API** для одного JSON-файла в `appDataFolder`.
