...
Закладки и настройки хранятся локально в `chrome.storage.local`. При входе в аккаунт Crypt-Chain данные синхронизации уходят **на `https://crypt-chain.com`** (см. `SERVER_API.md`). Синхронизация через Google Drive в расширении **не используется**.
