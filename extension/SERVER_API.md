...
```json
{
  "email": "user@example.com",
  "password": "secret",
  "device_id": "опционально: стабильный UUID клиента (до 128 символов)"
}
```

Поле **`device_id`** рекомендуется: на сервере для каждого устройства выдаётся отдельный токен; повторный вход с того же `device_id` отзывает только предыдущий токен этого устройства, другие ПК/браузеры остаются в сессии.
...
**Календарь:** флаг `settings.googleCalendarEnabled` синхронизируется вместе с остальными настройками: на другом устройстве после входа и pull клиент подхватывает «календарь включён» или «выключен». OAuth-токен Google хранится только в профиле Chrome на конкретном устройстве; при первом показе событий на новом ПК может понадобиться один раз пройти вход Google (если токена ещё нет).
...
При активной сессии сервера облачная копия через Google Drive **не используется** (источник правды — Crypt-Chain).
